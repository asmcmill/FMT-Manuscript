...
bch_nmds_stats <- bch_dist%>%
  ...
  left_join(.,samplekey%>%mutate(SUBJ.ID=as.character(SUBJ.ID)),by="CLIENT.IDENTIFIER")%>%
  column_to_rownames("CLIENT.IDENTIFIER")%>%
  pairwise.adonis2(as.dist(.[1:45]) ~ TIME.POINT.2, data = ., nperm = 1000, strata='SUBJ.ID')
...
nmdsplot(bch_nmds, "Metabolomics",2,8)
nmdsplot(genes_nmds, "Genes",2,8)
nmdsplot(tax_nmds, "Metagenomics",2,8)
...
ggplot(alpha, aes(x=TIME.POINT.2,y=shannon))+
  geom_dotplot(aes(fill=TIME.POINT.2),binaxis="y",stackdir="center",binwidth=5/50,dotsize=2,stroke=0.5)+
  geom_boxplot(outlier.alpha = 0,alpha=0,linewidth=.1)+
...
ggsave(paste0("Figures/Diversity/alpha_diversity.pdf"), height=2.3, width=3.25, units="in")
...
toptaxa<-phylo_by_time%>%
  ...
  mutate(count=1:n())%>%
  filter(simplename %in% c(as.character(fam_siglist$clade_name),"Akkermansiaceae"))%>% #significant and Akkermansia which doesn't significant change but makes up a big portion.
  ungroup()%>%
  ...
  mutate(simplename=factor(simplename,levels=names(anno_color$familyplot)))%>%
  droplevels()
...
ggsave("Figures/Diversity/stacked_taxonomy_barplot_by_time.pdf", width=3.5,height=2.5,units="in")
...
phylo_by_time%>%
  ungroup()%>%
  select(TIME.POINT.2,simplename,mean)%>%
  filter(simplename %in% fam_siglist$clade_name)%>%
  spread(TIME.POINT.2,mean)
```
# taxonomy RFA
```{r}
tax_RFA<-tax_raw%>%
...
tax_dir<-tax_raw%>%
  ...
  left_join(.,samplekey,by="CLIENT.IDENTIFIER")%>%
  select(clade_name, relabun,prepost)%>%
  group_by(prepost,clade_name)%>%
  summarize(mean=mean(relabun))%>%
  ...
  select(clade_name,dir)

set.seed(102309)
tax_forest<-tax_raw%>%
  mutate(taxlevel=str_sub(clade_name,regexpr("\\_{2}[^\\;]*$", clade_name)-1,regexpr("\\_{2}[^\\;]*$", clade_name)-1))%>%
  filter(clade_name %in% spec_siglist$clade_name)%>%
  column_to_rownames("clade_name")%>%
  select(-taxlevel)%>%
  t()%>%
  as.data.frame()%>%
...
#Prepare to Plot
tax_forest_plot<-tax_forest$importance%>%
  as.data.frame()%>%
  rownames_to_column("cd")%>%
  left_join(.,tax_dir%>%
              mutate(cd=janitor::make_clean_names(clade_name)),by="cd")%>%
  left_join(.,tax_metadata,by="clade_name")%>%
...
#plot
ggplot(data=tax_forest_plot%>%filter(count<=25), aes(y=MeanDecreaseAccuracy,x=simplename, fill=family))+
  ...
  theme(axis.text.y=element_text(size=6),
        ...
        legend.box.margin=unit(c(0,1,-.1,0),"in"))+
  guides(fill=guide_legend(ncol=2))

ggsave(paste0("Figures/Supplementals/species_RFA.pdf"), height=3.7, width=3, units="in")
...
```{r}
hm_tax<-tax_raw%>%
  filter(clade_name %in% (tax_forest_plot%>%filter(count<=25))$clade_name)%>%
  gather(key="CLIENT.IDENTIFIER",value="CPM",-clade_name)%>%
  left_join(.,samplekey,by="CLIENT.IDENTIFIER")%>%
  group_by(TIME.POINT.2,clade_name)%>%
  summarize(mean=log10(mean(CPM)+1))%>%
  spread(key=TIME.POINT.2,value=mean)%>%
  mutate(clade_name=factor(clade_name,levels=rev(unique((tax_forest_plot%>%filter(count<=25))$clade_name))))%>%
  arrange(clade_name)%>%
  column_to_rownames("clade_name")
...
pheatmap(hm_tax[1:4],
         ...
         cellwidth = 50,
         fontsize = 14,
         filename="Figures/Supplementals/tax_hm_mean.pdf",
         width=20,
         height=12
)
```
...
write_xlsx(list("1B"=tax_nmds,
                "1B pairwise adonis"=extractadonis(tax_nmds_stats,tax_dist),
                "1C"=bch_nmds,
                "1C pairwise adonis"=extractadonis(bch_nmds_stats,bch_dist),
                "1D"=alpha,
                "1D Wilcoxon signed rank"=alpha_stats,
                "1E"=phylo_by_time,
                "1E linear mixed model"=fam_siglist,
                "1F"=tax_forest_plot,
                "1F linear mixed model"=spec_siglist,
                "1G"=hm_tax),
           "Excel Exports/Figure 1.xlsx")

write_xlsx(list("S1"=phylo_all),"Excel Exports/Figure S1.xlsx")
