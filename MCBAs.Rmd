...
```{r}
#Correlation to Family
identical(rownames(MCBA_for_cor[,which(colnames(MCBA_for_cor) %in% MCBA_forest_plot$BA)]),rownames(fam_for_cor))

cor_MCBA_fam<-corr.test(MCBA_for_cor[,which(colnames(MCBA_for_cor) %in% MCBA_forest_plot$BA)],fam_for_cor,method="spearman",adjust="holm")

MCBA_fam_cor_plot<-left_join(as.data.frame(cor_MCBA_fam$p.adj)%>%
                              rownames_to_column("BA")%>%
                              gather(key="key",value="p",-BA),
                            as.data.frame(cor_MCBA_fam$r)%>%
                              rownames_to_column("BA")%>%
                              gather(key="key",value="r",-BA),by=c("BA","key"))%>%
  mutate(BA=factor(BA, levels=rev(MCBA_forest_plot$BA)))%>%
  filter(p<=0.05)%>%
  select(-p)%>%
  complete(BA,fill=list(key="drop",r=0))%>%
...
tax_anno<-tax_metadata%>%
  ...
  unique()%>%
  remove_rownames()%>%
  filter(phylum %in% names(anno_color$phylumplot))%>%
...
pheatmap(MCBA_fam_cor_plot,
         ...
         fontsize = 14,
         drop_levels = T,
         filename="Figures/MCBA/MCBA_vs_fam_spearman.pdf",
         annotation_colors = list(phylum=anno_color$phylumplot[names(anno_color$phylumplot) %in% tax_anno[colnames(MCBA_fam_cor_plot),"phylum"]]),
         width=14,
         height=17)

saveRDS(cor_MCBA_fam$p.adj,"Dataframes/Stats/MCBA vs family spearman t test holm.rds")
...
#list of aa significant by wilcoxon or linear modelling
...
aa_names<-c("Taurine","Glycine","Alanine","Isoleucine/Leucine","Valine","Phenylalanine","Tryptophan","Tyrosine","Serine","Glutamate","Histidine","Lysine")
names(aa_names)<-c("Tau","Gly","Ala","Ile_Leu","Val","Phe","Trp","Tyr","Ser" ,"Glu","His","Lys")

MCBA_boxplot_sort<-MCBA_boxplot%>%
         filter(Core %in%filter(MCBA_boxplot_stats,qval<0.05)$Core &
           Amino_Acid %in% names(aa_names))%>%
...
#Deconjugated 
(MCBA_siglist%>%left_join(.,MCBA_metadata,by="BA")%>%filter(Amino_Acid=="Deconjugated")%>%unique())$BA


#order and add bMCA as that's in the amino acid plots
decon_order<-c("CA","7-keto DCA","CDCA","DCA","12-oxo LCA","EDCA","LCA","3-oxo LCA", "isoLCA", "aMCA","bMCA", "wMCA","HCA", "HDCA")
...
ggsave("Figures/MCBA/MCBA_Boxplots_Unconjugated.pdf",width=5.6, height=1.6)

###Now everything for supp
ggplot(data=MCBA_boxplot%>%filter(Amino_Acid!="Deconjugated"),aes(x=Core,y=value,fill=prepost, group=interaction(Core,prepost)))+
  geom_jitter(shape=21,
              position=position_jitterdodge(dodge.width=0.75,jitter.width=0.2),
              size=0.8,stroke=0.1)+
  geom_boxplot(alpha=0,linewidth=0.2)+
  ...
  scale_fill_manual(values=anno_color$TIME.POINT.2)+
  stat_pvalue_manual(filter(MCBA_boxplot_stats,qval<0.05)%>%
                      filter(Amino_Acid!="Deconjugated")%>%
                      mutate(x=as.numeric(Core))%>%
                      mutate(xmin=x-0.2)%>%
                      mutate(xmax=x+0.2), 
                     label = "q.adj.signif",
...
ggsave("Figures/Supplementals/MCBA_Boxplots_all.pdf",width=8.25, height=11)
...
pathway_for_cor<-pathway_raw%>%
  ...
  separate(pathway, into=c("top","rest"), sep=";",extra="merge")%>%
  filter(str_detect(top,"Amino-acid_biosynthesis"))%>%
  gather(key="CLIENT.IDENTIFIER",value="CPM",-top,-rest)%>%
  group_by(rest,CLIENT.IDENTIFIER)%>%
  summarize(CPM=sum(CPM))%>%
  spread(key="CLIENT.IDENTIFIER",value="CPM")%>%
  select(1, samplekey$CLIENT.IDENTIFIER)%>%
  column_to_rownames("rest")%>%
  t()

identical(rownames(MCBA_for_cor),rownames(pathway_for_cor))

cor_MCBA_path<-corr.test(MCBA_for_cor[,which(colnames(MCBA_for_cor) %in% MCBA_forest_plot$BA)],pathway_for_cor,method="spearman",adjust="holm")

MCBA_path_cor_plot<-left_join(as.data.frame(cor_MCBA_path$p.adj)%>%
                              rownames_to_column("BA")%>%
                              gather(key="key",value="p",-BA),
                            as.data.frame(cor_MCBA_path$r)%>%
                              rownames_to_column("BA")%>%
                              gather(key="key",value="r",-BA),by=c("BA","key"))%>%
  filter(p<=0.05)%>%
  select(-p)%>%
...
pheatmap(MCBA_path_cor_plot,
         ...
         fontsize = 14,
         drop_levels = T,
         filename="Figures/MCBA/Sig_MCBA_vs_sig_aa_pathway_spearman.pdf",
         width=20,
         height=17)
```
...
identical(rownames(MCBA_for_cor),rownames(AA_for_cor))

cor_MCBA_AA<-corr.test(MCBA_for_cor,AA_for_cor,method="spearman",adjust="holm")

MCBA_AA_cor_plot<-left_join(as.data.frame(cor_MCBA_AA$p.adj)%>%
                               rownames_to_column("BA")%>%
                               gather(key="key",value="p",-BA),
                             as.data.frame(cor_MCBA_AA$r)%>%
                               rownames_to_column("BA")%>%
                               gather(key="key",value="r",-BA),by=c("BA","key"))%>%
  filter(p<=0.05)%>%
  select(-p)%>%
...
pheatmap(MCBA_AA_cor_plot,
         ...
         fontsize = 14,
         drop_levels = T,
         filename="Figures/MCBA/MCBA_vs_AA_spearman.pdf",
         width=14,
         height=17)

```
# Correlate with strains
```{r}
spec_for_cor<-tax_raw%>%
        filter(str_detect(clade_name,"\\;s\\_\\_*"))%>%
        #mutate(clade_name=str_remove(clade_name,".*\\;s\\_\\_"))%>%
        filter(clade_name!="")%>%
  column_to_rownames("clade_name")%>%
  t()%>%
  as.data.frame()

identical(rownames(MCBA_for_cor[,which(colnames(MCBA_for_cor) %in% MCBA_forest_plot$BA)]),rownames(spec_for_cor))

cor_MCBA_spec<-corr.test(MCBA_for_cor[,which(colnames(MCBA_for_cor) %in% MCBA_forest_plot$BA)],spec_for_cor,method="spearman",adjust="holm")

MCBA_spec_cor_plot<-left_join(as.data.frame(cor_MCBA_spec$p.adj)%>%
                               rownames_to_column("BA")%>%
                               gather(key="key",value="p",-BA),
                             as.data.frame(cor_MCBA_spec$r)%>%
                               rownames_to_column("BA")%>%
                               gather(key="key",value="r",-BA),by=c("BA","key"))%>%
  mutate(BA=factor(BA,levels=rev(MCBA_forest_plot$BA)))%>%
  filter(p<=0.05)%>%
  complete(BA,fill=list(key="drop",r=NA))%>%
  select(-p)%>%
  spread(key=key,value=r,fill=0)%>%
  column_to_rownames("BA")%>%
  select(-drop)
...
cor_anno<-as.data.frame(colnames(MCBA_spec_cor_plot))%>%
  dplyr::rename("clade_name"=1)%>%
  separate(clade_name,into=c("kingdom","phylum","class","order","family","genus","species"),sep="\\;.\\_\\_",remove=F)%>%
  column_to_rownames("clade_name")

pheatmap(MCBA_spec_cor_plot,
         annotation_col = cor_anno%>%select(phylum),
         labels_col=cor_anno$species,
         cluster_rows=F,
         cluster_cols=T,
         color=c(rev(colorRampPalette(c("grey88","white","dodgerblue3"),bias=1)(30)),colorRampPalette(c("grey88","white", "red3"),bias=1)(30)),
         breaks=seq(from=-1,to=1,by=2/60),
         ...
         fontsize = 14,
         drop_levels = T,
         annotation_colors = list(phylum=anno_color$phylumplot[names(anno_color$phylumplot) %in% cor_anno$phylum]),
         filename="Figures/MCBA/MCBA_vs_spec_spearman_RF.pdf",
         width=14,
         height=17)
