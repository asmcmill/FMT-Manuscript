...
ggplot(carn_by_time, aes(x=TIME.POINT.2,y=sum,group=TIME.POINT.2))+
  geom_dotplot(aes(fill=TIME.POINT.2),binaxis="y",stackdir="center",binwidth=100/50,dotsize=1.3,stroke=0.5)+
...
#Carnitine Metabolism Correlation
...
identical(rownames(carn_for_cor),rownames(bch_for_cor))

carn_cor<-corr.test(bch_for_cor%>%select(as.vector(sig_carn$BIOCHEMICAL)),carn_for_cor,method="spearman",adjust="holm")

carn_spec_cor_plot<-left_join(as.data.frame(carn_cor$p.adj)%>%
  rownames_to_column("BIOCHEMICAL")%>%
  gather(key="key",value="p",-BIOCHEMICAL),
  as.data.frame(carn_cor$r)%>%
  rownames_to_column("BIOCHEMICAL")%>%
...
pheatmap(carn_spec_cor_plot,
         ...
         width=5.4,
         height=5,
         filename=paste0("Figures/Carnitine/bch_vs_carn_spearman.pdf"))
