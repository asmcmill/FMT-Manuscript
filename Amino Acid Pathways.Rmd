...
pheatmap(pathway_aa%>%
         ...
         filename="Figures/Supplementals/amino acid biosynthesis.pdf",
         border_color="grey20",
         width=10,
         height=6)
...
pathway_aa_spec<-pathway_raw%>%
  ...
  filter(str_detect(pathway,"\\|"))%>%
  separate(pathway,into=c("type","amino","note", "species"),sep=";|\\.|\\|",extra="merge")%>%
  separate(amino,into=c("amino","bolism"),sep="_",extra="merge")%>%
...
ggplot(pathway_aa_spec%>%
  ...
  theme_bw()+
  facet_wrap(~amino,ncol=5,scales="free_y")+
...
ggsave("Figures/Supplementals/amino acid degradation_familyplot.pdf",height=3,width=8.25,units="in")

ggplot(pathway_aa_spec%>%
         filter(type=="Amino-acid_biosynthesis")%>%
         complete(family,fill=list(amino="D-alanine",type="Amino-acid_biosynthesis",CLIENT.IDENTIFIER="R1_1",CPM=0))
       ,aes(x=CLIENT.IDENTIFIER,y=CPM, fill=family))+
  geom_bar(stat="identity")+
  theme_bw()+
  facet_wrap(~amino,ncol=5,scales="free_y")+
...
ggsave("Figures/Supplementals/amino acid biosynthesis_familyplot.pdf",height=6,width=8.5,units="in")
```
# Species plots
```{r}
pathway_aa_spec<-pathway_raw%>%
